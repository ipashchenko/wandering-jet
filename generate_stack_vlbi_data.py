...
def generate_model_images(parallels_run_file, LOS_angels_rad, epochs, exec_dir, calculon=False):
    cwd = os.getcwd()
    # Construct params file
    with open(f"{parallels_run_file}", "w+") as fo:
        for epoch, los_angle_rad in zip(epochs, LOS_angels_rad):
            fo.write("{} {}".format(los_angle_rad, epoch))
            fo.write("\n")
    ...
    if calculon:
        n_jobs = 44
    os.system("parallel --files --results t_obs_{2}" + f" --joblog log --jobs {n_jobs} -a {parallels_run_file} -n 1 -m --colsep ' ' \"./bk_transfer\"")
    os.chdir(cwd)
...
# fig.savefig(os.path.join(save_dir, "precession.png"), bbox_inches="tight")
# plt.show()
...
# los_angle_rad_0 = np.deg2rad(0.75)
los_ange_deg_0 = 2.0
...
for i_epoch, (time, epoch) in enumerate(zip(times, epochs)):
    ...
    # # Run polarization transfer
    # os.chdir(jetpol_run_directory)
    # os.system("./bk_transfer {}".format(los_angle_rad))
    # os.chdir(cwd)

    # phis_prev = np.loadtxt(os.path.join(save_dir, "phis.txt"))
    # phis_prev = phis_prev.tolist() + phis
    # np.savetxt(os.path.join(save_dir, "phis.txt"), phis_prev)
